...
namespace eWorldCup.Controllers;
...
public class TournamentController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<Participant>>> GetParticipants()
    {
        ...
        if (participants == null || participants.Count == 0) 
            return NotFound(new ErrorResponse("No participants found"));
    ...
    public ActionResult<PairsResponse> GenerateRoundPairs([FromBody] GeneratePairsRequest request)
    {
        ...
        try
        {
            var pairs = _pairGenerator.GeneratePairs(request.Participants.AsReadOnly(), request.Round);
